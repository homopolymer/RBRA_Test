...
# gene alignment
gene_align = os.path.join(base_dir,'99_otus_aligned.fasta')
...
if os.path.exists('%s.gz'%gene_align):
    sys.stderr.write('%s\n'%(' '.join(['gzip','-d','%s.gz'%gene_align])))
    subprocess.call(['gzip','-d','%s.gz'%gene_align])
